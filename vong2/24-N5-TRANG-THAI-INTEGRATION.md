...
| `viva-asr` container | **Mô phỏng** *(nâng từ Kế hoạch, 04/08)* | Image **đã build và chạy thật**: `/health` 200, 36 clip tiếng Việt transcribe được, RTF median 0.167, `server_ms` p50=439/p95=667. Bằng chứng: `evidence/asr/` | Chạy trên **CarSky** (push Zot + Container Node — V7). Số hiện tại đo trên CPU máy dev, không phải node CarSky |
